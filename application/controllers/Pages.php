...
*/
	...
	function request_me2($parameter=array()) {
		...
		if($user_details->county_code==''){$user_details->county_code='NA';}
         if($user_details->county_code=='966'){$user_details->county_code='SA';}
    ...
    function request_me2MADA($parameter=array()) {
		extract($parameter);
		...
		if($user_details->region==''){$user_details->region='NA';}
		if($user_details->state==''){$user_details->state='NA';}
		if($user_details->county_code==''){$user_details->county_code='SA';}
       if($user_details->county_code=='966'){$user_details->county_code='SA';}
		if($user_details->zip_code==''){$user_details->zip_code='NA';}
	//	"&amount=".$parameter['amount'].
		$rand = mt_rand(100000, 999999);
...
function get_hyperPayMADA()
{
    ...
    $amount=$package_details->price;
    $parameter['amount']= $amount;
	$responseData = self::request_me2MADA($parameter);
	 $responseData=json_decode($responseData);
   // print_r($responseData);
	$data['ini_payment']=$responseData;
	//$this->load->view('website/include/header.php', $data);
	$this->load->view('website/payment-pageMADA.php', $data);
	//$this->load->view('website/include/footer.php', $data);
}
...
function getPayStatus_test()
	...
	$result=json_decode($responseData);
 	...
 	if((preg_match('/^(000\.000\.|000\.100\.1|000\.[36])/', $code)==1) || (preg_match('/^(000\.400\.0|000\.400\.100)/', $code)==1))
    {
    ...
    {
        echo ("<script LANGUAGE='JavaScript'>
            window.alert('Transaction failed. Please try again after some time! Code:$code');
