...
class Web_modul extends CI_Model
{
    ...
    function article_list($type=null,$start_from=0,$upto=1,$userID=Null){
        ...
        {
          $this->db->select('article.id,article.title,article.image,article.id,LEFT(article.description,100) as description,article.created_on,');
    ...
    function news_list($type=null,$start_from=0,$upto=1,$userID=Null){
        ...
        {
          $this->db->select('Lnews.id,Lnews.title,Lnews.image,LEFT(Lnews.description,100) as description,Lnews.created_on');
           $this->db->select('IF(BOOK.id, "1", "0") as staus_book');
            $this->db->limit($upto,$start_from);     
        }              
    ...
    function useful_link_list($type=null,$start_from=0,$upto=1){
        ...
        {
          $this->db->select('UseLink.id,UseLink.title,LEFT(UseLink.description,100) as description,UseLink.created_on,UseLink.links');
            $this->db->limit($upto,$start_from);     
        }              
                   
          $this->db->from('useful_links as UseLink');
          $this->db->order_by('UseLink.title','ASC');
        $result = $this->db->get();
        if($type=='count')
        {
         return  $result->num_rows();
    ...
    function useful_document_list($type=null,$start_from=0,$upto=1){
        ...
        {
          $this->db->select('legalDoc.id,legalDoc.title,legalDoc.created_on,legalDoc.download_link');
            $this->db->limit($upto,$start_from);     
        }              
    ...
    function faq_list($type=null,$start_from=0,$upto=1){
        ...
        {
          $this->db->select('faqs.id,faqs.title,faqs.created_on,faqs.description');
            $this->db->limit($upto,$start_from);     
        }              
